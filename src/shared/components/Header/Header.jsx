...
import { faArrowRightFromBracket,faUsersBetweenLines } from '@fortawesome/free-solid-svg-icons';
...
export default function Homepage() {
	...
	useEffect(() => {
		document.addEventListener('click', (e) => { 
			if(e.target !== dropDownLinkRef.current)
				setShowDropdown(false)
		})
	}, [])

	return(
		<header className="mb-8 w-full h-16 shadow-bottom shadow-sm shadow-slate-700/50 text-slate-700 transition-all">
			<div className='max-w-5xl h-full mx-auto px-4 flex justify-between align-center'>
				<Link to='/' className='flex justify-center items-center cursor-pointer'>
					<div className='mr-3 text-3xl font-bold text-[rgb(40,40,40)] inline-block'>BlueCobalt</div>
				...
				<nav>
					<ul 
						...
						onClick={(e) => {
							const styleClasses = ['border-b-2', 'border-md','border-sky-700']
							const li = (e.target.tagName === 'LI') ? e.target : e.target.closest('li');
							if(linkRef.current === null) {
								styleClasses.forEach(className => li.classList.add(className))
								linkRef.current = li
							}
							else {
								// remove previous active link style
								styleClasses.forEach(className => linkRef.current.classList.remove(className))
								styleClasses.forEach(className => li.classList.add(className))
								linkRef.current = li
							}
					}}>
						{
							...
							: <>
								...
								<li className='relative drop-shadow-lg z-50 w-10 h-10 bg-slate-400 rounded-full' 
									ref={dropDownLinkRef}
									onClick={() => {
										setShowDropdown(prevState => !prevState)
									}}
								>
									<div className={ `${(showDropdown) ? '' : 'hidden' } bg-white drop-shadow-lg rounded-lg absolute top-14 right-0 w-60 ease-in-out` }>
										<DropdownItemLink to='/profile' leftIcon={<FontAwesomeIcon icon={faUser} />}>Profile</DropdownItemLink>
										<DropdownItemLink to='/profile' leftIcon={ <FontAwesomeIcon icon={faUsersBetweenLines} /> }>Manage Class</DropdownItemLink>
										<DropdownItemLink 
